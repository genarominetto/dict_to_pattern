...
from component.abstract.graphic_modules.data_loader import DataLoader
from component.abstract.graphic_modules.graphic_validator import GraphicValidator  # Import the new validator


class Graphic(ABC):
    """
    Abstract base class for all graphical components (both composite and leaf).
    """
    ...
    def __init__(self, name: str):
        ...
        self._id = Graphic._generate_id()  # Automatically assign a unique id to each instance
        self._children: List['Graphic'] = []  # Private children property
        self.name = name
        self._validator = GraphicValidator(self)  # Initialize the validator with the current Graphic instance
    ...
    def _generate_id(cls) -> int:
        """Generates a new unique id for each Graphic instance."""
        cls._id_counter += 1
    ...
    def is_composite(self) -> bool:
        """Returns True if this component is a composite (group)."""
    ...
    def execute_operation_recursively(self, operation: Callable[['Graphic'], None]) -> None:
        """Recursively execute an operation on this graphic and all its children."""
        operation(self)
        for child in self.get_children():
    ...
    def validate_structure(self, 
        ...
        """
        Validates the structure of the graphic component by delegating to the GraphicValidator.
    ...
    def export_to_pickle(self, path: str) -> None:
        """Exports the graphic structure to a pickle file."""
        self._loader.export_to_pickle(self, path)

    @classmethod
    def import_from_pickle(cls, path: str) -> 'Graphic':
        """Imports a graphic structure from a pickle file."""
    ...
    def clone(self) -> 'Graphic':
        ...
        cloned_graphic = self.__class__.__new__(self.__class__)
        
        for attr_name, attr_value in self.__dict__.items():
            if attr_name == '_id':
                setattr(cloned_graphic, '_id', Graphic._generate_id())
            else:
                setattr(cloned_graphic, attr_name, attr_value)
        
        cloned_graphic._children = []
        for child in self.get_children():
            cloned_child = child.clone()
            cloned_graphic.add(cloned_child)
        
        cloned_graphic._parents = []
