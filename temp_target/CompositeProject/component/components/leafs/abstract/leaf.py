...
class Leaf(Graphic):
    ...
    def __str__(self, level=0) -> str:
        indent = "    " * level
        return f"{indent}{self.name} (size: {self.size}, active: {self.is_active})"
    ...
    def get_components_recursively(
        ...
        # Check if the condition function is provided, if not, return the circle itself
        if condition_func:
            # If condition_func evaluates to True, return the circle
            if condition_func(self, *condition_args):
                return [self]
            else:
                return []  # Return empty if it doesn't meet the condition
        return [self]  # If no condition, just return the circle itself
