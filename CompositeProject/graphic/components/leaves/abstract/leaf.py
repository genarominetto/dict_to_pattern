...
class Leaf(Graphic):
    ...
    def __str__(self, level=0) -> str:
        indent = "    " * level
        return f"{indent}{self.name} (size: {self.size}, is_active: {self.is_active})"
    ...
    def remove_components_recursively(
        self, condition_func: Callable[[Graphic, Any], bool],
        condition_args: Union[tuple, Any] = ()
    ) -> List[Graphic]:
        return []

    def get_components_recursively(
        self, condition_func: Optional[Callable[[Graphic, Any], bool]] = None,
        condition_args: Union[tuple, Any] = ()
    ) -> List[Graphic]:
        # Check if the condition function is provided, if not, return the leaf itself
        if condition_func:
            # If condition_func evaluates to True, return the leaf
            if condition_func(self, *condition_args):
                return [self]
            else:
                return []  # Return empty if it doesn't meet the condition
        return [self]  # If no condition, just return the leaf itself

    def execute_operation_recursively(
        self, operation_func: Callable[[Graphic, Any], Any],
