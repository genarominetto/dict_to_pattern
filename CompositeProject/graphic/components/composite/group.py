...
class Group(Graphic):
    ...
    validation_settings: Dict[str, bool] = {
        ...
        'only_component_objects_in_composites': True,
        'all_parents_are_composites': True,
        'leaf_has_no_children': True,
        'components_are_unique': True,
        'ids_are_unique': True,
        'names_are_unique': True,
        'all_composites_use_deny_policy': True,
        'max_one_parent': True,
        'condition_func_in_all_conditional_composites': True,
    }
    ...
    def __init__(
        ...
        self.duplicate_policy = duplicate_policy or self.DuplicatePolicy.DENY_MULTIPLE_PARENTS
        self.condition_func = condition_func

        if self.duplicate_policy == self.DuplicatePolicy.ALLOW_NEW_PARENT_IF and not self.condition_func:
    ...
    def _validate_composite_logic(self, component: Graphic) -> None:
        ...
        if self.duplicate_policy == self.DuplicatePolicy.DENY_MULTIPLE_PARENTS and component.get_parents():
            raise ValueError(f"Error: {self.name} has the policy DENY_MULTIPLE_PARENTS, "
                                     "and this component already has a parent.")

        if self.duplicate_policy == self.DuplicatePolicy.ALLOW_NEW_PARENT_IF:
    ...
    def add(self, component: Graphic) -> None:
        ...
        component._parents.append(self)  # Add the current composite to the parents
    ...
    def remove(self, component: Graphic) -> None:
        ...
        if component in self.get_children():
            component._parents.remove(self)  # Remove this composite from the component's parents
    ...
    def remove_components_recursively(
        ...
        for child in self.get_children()[:]:
            ...
            if condition_func is None or condition_func(child, *condition_args):
                child._parents.remove(self)  # Remove this composite as parent before removing
    ...
    def calculate_depth(self) -> int:
        return 1 + max(child.calculate_depth() for child in self.get_children()) if self.get_children() else 1
    ...
    def export_to_pickle(self, path):
        pass
